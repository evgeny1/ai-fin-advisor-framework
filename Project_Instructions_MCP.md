...
- `advisor_write_back(...)` — §8 entry + Portfolio_State git commit
  ⚠ **ENG-48 fix (2026-07-14):** this call is wrapped in a 90s safety timeout, and the underlying write can genuinely finish just past that deadline (confirmed race, ~90.02s observed). The tool now verifies via `git rev-parse HEAD` before reporting failure: a delayed-but-real success returns `{"status": "OK_DELAYED", "committed": true, "commit_hash": ...}` — treat this exactly like a normal successful write-back, not a failure. A genuine `{"status": "TIMEOUT"}` still means what it always has (ENG-49): check `git log`/`git status` for a real commit before retrying rather than assuming success or failure either way.
- `advisor_evaluate_trend_signal()` — ENG-50/ENG-55 Step 6c: deterministic trend/rotation signal for the 8 held instruments with an ENG-55 comparator (MLPX, DBMF, XAR, AIPO, COPX, SGOL, SIVR, MAGS). Shadow-mode only — **NEVER feeds M03, NEVER overrides an allocation directive.** Call after `advisor_apply_scoring()` (needs cached scenario_probs), automatically every FULL_DESKTOP session. Logs to `TrendSignalStore.json` alongside `dominant_directive_conflict_aware` for the ~8-week trial checkpoint, and retroactively fills any forward-outcome check now due (~21 trading days after a prior signal). Returns `trend_signals` (per ticker: `rs_signal` STRENGTHENING/WEAKENING/INCONCLUSIVE/DATA_UNAVAILABLE, `own_short_dir`/`own_medium_dir`, `comparator_mode`/`comparator_detail`, `dominant_directive_conflict_aware`, `margin_debt_fragility_flag` [null until ENG-54 is wired], `quality_flags`) and `store_summary`. **ENG-60 (2026-07-08):** `rs_signal` has a 4th value, `DATA_UNAVAILABLE`, distinct from `INCONCLUSIVE` — a required input (comparator series, macro confirmation gate, or own-price history) was missing this session, so the computation never ran to a real verdict at all. `INCONCLUSIVE` now means the computation ran on sufficient data and genuinely found no resolved direction (opposite-sign windows, or a spread below the noise floor). Both used to collapse into `INCONCLUSIVE`; `quality_flags` always explains which one and why. Present STRENGTHENING/WEAKENING reads in the briefing's new TREND_ROTATION_SIGNAL section (informational only, like M14's MARKET_REGIME_SIGNAL) — see Step 7 below.
...
7. Produce briefing           → write M04-format briefing in conversation
                                ...
                                TREND_ROTATION_SIGNAL: informational only, like
                                  ...
                                  STRENGTHENING/WEAKENING/INCONCLUSIVE/DATA_UNAVAILABLE
                                  reads per instrument; NEVER frame as an execution
                                  directive or compare against
                                  dominant_directive_conflict_aware as if the two should
                                  agree — they're independent signals by design during
                                  the ~8-week shadow-mode trial.
                                  ENG-60 (2026-07-08): present DATA_UNAVAILABLE reads
                                  separately from INCONCLUSIVE reads, not lumped
                                  together — e.g. "No read (data unavailable): DBMF,
                                  SGOL, SIVR" as its own line distinct from "Inconclusive
                                  (no sustained divergence): XAR, AIPO, MAGS". The
                                  distinction is the entire point of ENG-60; collapsing
                                  them back together in the briefing defeats it even if
                                  the underlying data is now correct.
...
9. Write-back                 → advisor_write_back(primary_driver, open_triggers,
                                ...
                                status == "OK_DELAYED" (ENG-48 fix, 2026-07-14): treat as
                                  a normal success — the write finished, just after the
                                  90s safety-timeout deadline; the tool verified a real
                                  git commit before returning this instead of a false
                                  TIMEOUT. Only a genuine status == "TIMEOUT" means check
                                  git log/git status before retrying (ENG-49).
...
→ advisor_evaluate_allocation(account_id, current_weights, objective_type,
                                planning_horizon_years, tickers=[asset],
                                proposed_allocations, owner?,
                                floor_nominal_loss?, concentration_cap?,
                                drawdown_tolerance?)
   ⚠ flat scalar params, NOT a single account_profile dict — see the
   MCP mode bullet list above (ENG-33) for why
...
- NEVER present a DATA_UNAVAILABLE read as if it were a computed INCONCLUSIVE finding, or vice versa (ENG-60, 2026-07-08) — DATA_UNAVAILABLE means a required input was missing this session (no basis to compute anything); INCONCLUSIVE means the computation ran on sufficient data and genuinely found no resolved direction. Collapsing the two defeats the reason ENG-60 split them apart.
