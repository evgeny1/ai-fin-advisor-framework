...
**Moved to `Instrument_Classification.md`** (ENG-51, 2026-07-06) -- role registry + instrument classification table now live in their own file, fetched via `M12_DriveProtocol.fetchInstrumentClassification()`. Section number kept stable here (no renumbering of §12/§13) so every existing "§11.x" cross-reference in the framework still resolves -- just in the other file now.
